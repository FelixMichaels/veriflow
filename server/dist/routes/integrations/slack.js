...
async function handleVeriFlowCommand(text, userId, userName, channelId, res) {
...
async function createVerificationRequest(args, userId, userName, res) {
    const email = args[0];
...
async function showScheduleMeetingModal(payload, sessionId, res) {
    const modal = {
        type: 'modal',
...
function verifySlackSignature(req) {
    ...
    if (!signature || !timestamp)
        return false;
    const hmac = crypto_1.default.createHmac('sha256', process.env.SLACK_SIGNING_SECRET);
...
async function openSlackModal(triggerId, modal) {
...
async function scheduleMeeting(sessionId, meetingData) {
