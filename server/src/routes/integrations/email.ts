...
import { mockSessions, mockUsers, refreshSessions } from '../verification';
...
async function processIncomingEmailForTenant(emailData: any, tenantId: string) {
...
async function extractVerificationRequest(emailData: any) {
    ...
    const isVerificationRequest = verificationKeywords.some(keyword => 
...
async function extractEmployeeInfo(emailData: any) {
...
function classifyRequestType(text: string): string {
    if (text.includes('password')) return 'password_reset';
    if (text.includes('mfa') || text.includes('2fa')) return 'mfa_reset';
    if (text.includes('locked') || text.includes('login')) return 'account_unlock';
    return 'general_verification';
}

async function sendConfirmationEmail(to: string, session: any) {
...
async function parseMailgunWebhook(data: any) {
    return {
        from: data.sender,
        to: data.recipient,
...
async function parseSendGridWebhook(data: any) {
    return {
        from: data.from,
        to: data.to,
        subject: data.subject,
...
async function sendTenantConfirmationEmail(to: string, session: any, tenantId: string) {
    const tenant = await getTenantConfig(tenantId);
...
async function createVerificationSession(data: any) {
    ...
    const session = {
        ...
        meetingDate: '',
        meetingTime: '',
        meetingStatus: 'not_scheduled' as const,
        meetingId: '',
        meetingAgenda: '',
    ...
    refreshSessions(); // Refresh in-memory sessions
